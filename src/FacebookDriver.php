...
class FacebookDriver extends HttpDriver implements VerifiesService
{
    ...
    {
        if ($request->get('hub_mode') === 'subscribe' && $request->get('hub_verify_token') === $this->config->get(
                'verification'
            )) {
    ...
    public function getConversationAnswer(IncomingMessage $message)
    {
        ...
        if (isset($payload['message']['quick_reply'])) {
            return Answer::create($payload['message']['text'])->setMessage($message)->setInteractiveReply(
                true
            )->setValue($payload['message']['quick_reply']['payload']);
        } elseif (isset($payload['postback']['payload'])) {
            return Answer::create($payload['postback']['title'])->setMessage($message)->setInteractiveReply(
                true
    ...
    public function getUserWithFields(array $fields, IncomingMessage $matchingMessage)
    {
        ...
        $userInfoData = $this->http->get(
            $this->facebookProfileEndpoint . $matchingMessage->getSender(
